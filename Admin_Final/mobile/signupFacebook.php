	...
	if(isset($_REQUEST['postdata'])){
		...
		try{
			...
			if($testUser == true ){
	    		...
	    		(name, email, avatar, password, login_status, last_login_date, is_authorised, date_of_birth,sex,social,social_id)
	    		VALUES (
	    				'". $post->name ."', 
	    				'". $post->email ."', 
	    				'". $post->avatar ."',
				...
				$user = getUserOfFacebookById($post->password);
			}
