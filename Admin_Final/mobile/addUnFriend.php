<?php
	...
	if(count($errors) == 0 && isset($_REQUEST['postdata'])){
		...
		try{
			...
			//add a comment
			$sql = "DELETE FROM user_friend WHERE friend_id = ". $user["id"] . " AND user_id = " . $post->user_id;
    		
			$conn->exec($sql);

			//add a comment
			$sql = "DELETE FROM user_friend WHERE friend_id = ". $post->user_id . " AND user_id = " . $user["id"];
